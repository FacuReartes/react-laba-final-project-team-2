...
it('User should be able to add a product', async () => {
  ...
  fireEvent.input(screen.getByPlaceholderText('Do not exceed 300 characters.'), {
    target: {
  ...
  await waitFor(() => {
    ...
    expect(screen.getByText(/New product has been added successfully/i)).toBeInTheDocument()
  })
})
...
describe('Form Validation Errors', () => {
  ...
  it('Product is too long', async () => {
    ...
    fireEvent.submit(screen.getByTestId('submit'))
    await waitFor(() => {
      expect(screen.queryByText(/Product name is too long/)).toBeInTheDocument()
    })
  })
...
describe('Image handling', () => {
  it('User should be able to delete an image', async () => {
    ...
    // Modal opens
    expect(screen.getByText(/Are you sure to delete product image\?/i)).toBeInTheDocument()
    ...
    expect(screen.queryByText(/Are you sure to delete product image\?/i)).toBeFalsy()
