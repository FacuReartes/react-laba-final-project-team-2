...
describe('Recently Viewed Tests', () => {
  ...
  it('Product should be correctly stored in browser storage when viewed', () => {
    ...
    expect(localStorage.setItem).toHaveBeenCalledWith('recently-watched', JSON.stringify(mockProduct));

  })
    
  it('User should be able to see all recently viewed products', () => {
    // Mock a list of 3 products and save them to storage
    const productList = Array(3).fill('').map((product, index) => {
  ...
  it('User should be able to see empty state', () => {
    ...
    expect(screen.queryByText(/You haven't watched any products recently./i)).toBeInTheDocument()
  ...
  it('Old products should be deleted when reaching the product limit', () => {
    ...
    const productList = Array(12).fill('').map((product, index) => {
      product = {...mockProduct[0]}
      product.id = index
      return product
    })
    ...
    expect(JSON.parse(localStorage.getItem('recently-watched')!)).toHaveLength(12)

    renderWithContext(<ProductDetail params={1}/>)

    const newProductList = JSON.parse(localStorage.getItem('recently-watched')!)
    // Length stays the same, and the last item (id = 11) has been erased
    expect(newProductList).toHaveLength(12)
    expect(newProductList.find((product: { id: number }) => product.id === 11)).toBeFalsy()
  ...
  it('Products should not be repeated', () => {
    ...
    expect(JSON.parse(localStorage.getItem('recently-watched')!)).toHaveLength(1)
  })
